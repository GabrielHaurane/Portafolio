...
5. **Colores solo por tokens, en los dos temas.** Toda la paleta vive en `src/App.css` como `--color-*`: tema oscuro (por defecto) en `:root` y tema claro en `[data-theme="light"]`. Todo token nuevo se define en **ambos** bloques. Ningún hex nuevo en JSX ni CSS, y nada de colores fijos de Bootstrap que asuman un fondo (`text-white`, `btn-outline-light`, `color: white`).
